...
Delay.hasMany(ReceiverWallet, {foreignKey: {name:'delay', defaultValue: '-1'} , as: 'receiver_wallet', foreignKeyConstraint: 'delay'})
ReceiverWallet.belongsTo(Delay,{foreignKey: {name:'delay', defaultValue: '-1'}, as: 'delay_obj' , foreignKeyConstraint: 'delay'})
setTimeout(async ()=>{
    await Delay.destroy({truncate: true})
    await Delay.bulkCreate(require('../utils/constants/delays').delays)
    const depositWallets =  require('../utils/constants/deposit_wallets').map(wallet=>{return {wallet_address: wallet}})
