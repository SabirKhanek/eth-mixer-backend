...
class WalletAllocationService {
  ...
  async getReceiverWalletFromDb(receiver_address) {
    ...
    const receiver_wallet_request = await ReceiverWallet.findAll({
      where: query,
      include: { model: Delay, as: "delay_obj" },
    });
  ...
  async isTxnExists(txn_hash) {
    try {
      return (await ProcessedTxns.findOne({ where: { txn_hash } }))?.dataValues
        .txn_hash;
    } catch (err) {
      return null;
    }
  ...
  async deleteStaleMixerRequests() {
    try {
      ...
      XMinutesAgo.setMinutes(
        XMinutesAgo.getMinutes() - CONFIG.MIXER_REQUEST_STALE_THRESHOLD
      );
  ...
  async getDepositAddress() {
    ...
    depositAddressMap.sort(
      (a, b) => a.receiver_wallets_length - b.receiver_wallets_length
    );
